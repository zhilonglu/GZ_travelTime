...
# txt => csv
link_info = pd.read_table(datapath+"gy_contest_link_info.txt",sep=';')
link_info = link_info.sort_values('link_ID')
training_data = pd.read_table(datapath+"gy_contest_link_traveltime_training_data.txt",sep=';')
print(training_data.shape)
training_data = pd.merge(training_data,link_info,on='link_ID')
testing_data = pd.read_table(datapath+"submit_totalWeighted.txt",sep='#',header=None)
...
print(feature_date)
feature_date.to_csv(datapath+"pre_data\\feature_data.csv",index=False)

feature_data = pd.read_csv(datapath+"pre_data\\feature_data.csv")
feature_data_date = AddBaseTimeFeature(feature_data)
print(feature_data_date)
feature_data_date.to_csv(datapath+"pre_data\\feature_data.csv",index=False)
...
sub_demo = pd.read_table(datapath+"submit_totalWeighted.txt",header=None,sep='#')
...
tt[['link_ID','date','time_interval','travel_time']].to_csv(datapath+"提交2017-08-06.txt",sep='#',index=False,header=False)
